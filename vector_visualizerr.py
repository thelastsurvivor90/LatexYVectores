...
import numpy as np
from pylatex import Document, Section, Subsection, Math, Package # pyright: ignore[reportMissingImports]
from pylatex.utils import NoEscape # pyright: ignore[reportMissingImports]
import os
...
class VectorDocument:
    ...
    def __init__(self, title="Visualización de Vectores"):
...
def ejemplo_vectores_3d():
    ...
    \\\\[0.3cm]
    \\textbf{{Verificación de perpendicularidad:}}
    \\\\
...
def ejemplo_completo():
    ...
    doc = VectorDocument(title="Sistema Completo de Visualización Vectorial")
    ...
    # PARTE 1: VECTORES 2D
    doc.doc.append(Section("Álgebra Vectorial en 2D"))
    ...
    # Producto escalar
    doc.doc.append(Subsection("Multiplicación por Escalar"))
    u2 = u * 1.5
    doc.add_vector_2d(u2, 'vec4', title="$1.5\\vec{u}$")
    
    # PARTE 2: VECTORES 3D
    doc.doc.append(NoEscape(r'\newpage'))
    doc.doc.append(Section("Álgebra Vectorial en 3D"))
    ...
    doc.doc.append(NoEscape(r'\newpage'))
    doc.doc.append(Section("Aplicaciones Prácticas"))
    
    doc.doc.append(Subsection("Física: Vectores de Fuerza"))
    f1 = Vector2D(3, 4, "F_1")
    f2 = Vector2D(-2, 1, "F_2")
...
if __name__ == "__main__":
    print("=" * 60)
    print("VectorVisualizer - Sistema de Visualización de Vectores")
    print("=" * 60)
    print()
    ...
    if opcion == "1":
        ...
        print("Opción no válida. Generando ejemplo completo...")
        ejemplo_completo()
